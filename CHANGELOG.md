...
### Added
- **제품 상태에 [배포일] 추가** — 기존 [완료]/[최종] 옆에 새 상태. 노란색 뱃지(`#facc15` 배경 + 진한 글자)로 표시
- 캘린더 타일 / 모달 일정 리스트의 토글 버튼 / 일정표 복사 / 사내 게시판용 복사 / JSON 가져오기까지 일관 적용

### Changed
- **일정 정렬 4단계로 확장**:
  1. `planned` 여부 — 일반 일정 위, 🔒 예정 일정 아래
  2. 회사 색상 (0→3, 지란지교소프트→기타)
  3. 배포일 여부 — 같은 회사 그룹 내에서 배포일 상태는 마지막
  4. 텍스트 가나다/알파벳순 (ko 로케일, 숫자 자연 정렬)
